...
class Main 
{
	...
	static function main() 
	{
		...
		var grid = processGrid(file, output);
	...
	static function processGrid(file:String, output:String):Array<Array<String>>
	{
		...
		Utf8.iter(file, function(char:Int){
			switch(char){
				case 0x0A://LINE FEED
					if (!cellStarted) {
						//ignore line feeds outside of a cell
					}
					else {
						addChar(char);
					}
				case 0x0D://CARRIAGE RETURN
					...
					if (cellStarted) {					//encountered a carriage return inside of a cell
